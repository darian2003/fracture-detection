...
class ThresholdMarginLoss(nn.Module):
    def __init__(self, base_loss, margin=0.2, threshold=0.5, margin_weight=0.5):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        # Penalize predictions close to the threshold (encourage confident decisions)
        distance_from_thresh = torch.abs(probs - self.threshold)
        margin_penalty = torch.relu(self.margin - distance_from_thresh)

        # Average margin loss over valid samples
        margin_loss = margin_penalty.mean()

        return base + self.margin_weight * margin_loss


def main(backbone='densenet169', threshold=0.5, lr=0.0001, batch_size=8, agg_type='prob_mean', alpha=0.75, gamma=2.0, num_epochs=20, margin=0.15, margin_weight=0.5, patience=3, seed=42):
    ...
    # Compute abnormal and normal counts for custom Weighted BCE
    df_train = pd.read_csv(train_csv)
    num_abnormal = df_train['label'].sum()
    num_normal = len(df_train) - num_abnormal

    # Define custom Weighted BCE Loss
    # Assume num_abnormal and num_normal are precomputed
    base_loss = WeightedBCELoss(abnormal_count=num_abnormal, normal_count=num_normal).to(device)
    criterion = ThresholdMarginLoss(base_loss, margin=margin, threshold=threshold, margin_weight=margin_weight).to(device)
...
if __name__ == "__main__":
    ...
    model, test_results, best_threshold = main(threshold=0.5, agg_type='prob_mean', alpha=0.75, gamma=2.0, num_epochs=20, margin=25, margin_weight=0.5, patience=4, seed=42)
