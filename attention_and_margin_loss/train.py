...
class AttentionMURAClassifier(nn.Module):
    def __init__(self, backbone='resnet50', pretrained=True, agg_strategy='attention', threshold=0.5):
        super(AttentionMURAClassifier, self).__init__()
    ...
    def extract_features(self, x):
        ...
        # Embed features into a consistent representation
        embedded = self.feature_embedding(features)
        return embedded

    def compute_attention(self, features, mask):
        """
        Compute attention weights for the image features
        
        Args:
            features: Tensor of shape [batch_size, max_images, feature_dim]
            mask: Tensor of shape [batch_size, max_images, 1]
            
        Returns:
            context: Tensor of shape [batch_size, feature_dim]
            attention_weights: Tensor of shape [batch_size, max_images, 1]
        """
        # Calculate attention scores
        attention_scores = self.attention(features)  # [batch_size, max_images, 1]
        ...
        # Multiply each feature vector by its attention weight
        weighted_features = features * attention_weights
        
        # Sum weighted features across images dimension
        context = weighted_features.sum(dim=1)  # [batch_size, feature_dim]
        
        return context, attention_weights

    def forward(self, x_list):
        """
        Forward pass with attention mechanism for image interaction
        ...
        Returns:
            ...
            image_logits: Tensor of shape [batch_size, max_images, 1]
            attention_weights: Tensor of shape [batch_size, max_images, 1]
            masks: Tensor of shape [batch_size, max_images, 1]
        """
        ...
        # Extract features from all images at once
        batch_features = self.extract_features(batch_images)
        
        # Reshape back to [batch_size, max_images, feature_dim]
        batch_features = batch_features.view(batch_size, max_images, -1)
        
        # Apply attention to get context vector and attention weights
        context, attention_weights = self.compute_attention(batch_features, masks)
        
        # Final classification for the study
        study_logits = self.classifier(context)
        
        # Calculate per-image logits for individual image loss (optional)
        # We'll use the same classifier for consistency
        image_features = batch_features.view(-1, batch_features.size(-1))
        image_logits = self.classifier(image_features)
        image_logits = image_logits.view(batch_size, max_images, -1)
        
        # Mask out padding for image logits
        image_logits = image_logits * masks
        
        return study_logits, image_logits, attention_weights, masks
...
def train_attention_model(model, train_loader, val_loader, criterion, optimizer, scheduler=None,
    ...
    for epoch in range(num_epochs):
        ...
        # Validation phase
        val_metrics = evaluate_attention_model(model, val_loader, criterion, device, threshold)
...
def evaluate_attention_model(model, data_loader, criterion, device='cuda', threshold=0.5):
    """
    Evaluation function for attention-based model
    """
    model.eval()
    val_loss = 0.0
    val_outputs = []
...
def visualize_attention_weights(model, data_loader, num_samples=5, device='cuda'):
    """
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            for i in range(min(len(images_list), num_samples - samples_seen)):
                ...
                plt.tight_layout()
                plt.savefig(f"attention_sample_{samples_seen}.png")
                plt.close()
                
                samples_seen += 1
                
    print(f"Saved {samples_seen} attention visualization plots.")